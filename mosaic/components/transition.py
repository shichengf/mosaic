...
class NPChangeTransitionPrior(nn.Module):
    ...
    def forward(self, x, embeddings, masks=None):
        ...
        x = torch.swapaxes(x, 2, 3)
        shape = x.shape
        # Repeat embeddings to match unfolded time steps: [BS, hidden] -> [BS*(T-L), hidden]
        num_windows = length - self.L
        embeddings = embeddings.unsqueeze(1).repeat(1, num_windows, 1).reshape(-1, embeddings.shape[-1])
        x = x.reshape(-1, self.L+1, input_dim)
        xx, yy = x[:,-1:], x[:,:-1]
        yy = yy.reshape(-1, self.L*input_dim)
        residuals = [ ]
